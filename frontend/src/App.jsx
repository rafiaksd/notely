...
function NoteItem({ note, onToggleComplete, onDelete, onMoveUp, onMoveDown, onUpdate }) {
  const [isEditing, setIsEditing] = useState(false);
  const [editTitle, setEditTitle] = useState(note.title);

  const saveEdit = async () => {
    if (editTitle.trim() && editTitle !== note.title) {
      await onUpdate(note.id, editTitle);
    }
    setIsEditing(false);
  };
...
function App() {
  ...
  const createNote = async (e) => {
    ...
    setTitle("");
    setNotes((prev) => [...prev, res.data].sort((a, b) => a.position - b.position));
  };
  ...
  const toggleComplete = async (id) => {
    ...
    await api.patch(`notes/${id}/`, { completed: !note.completed });
    setNotes((prev) =>
      prev
        .map((n) => (n.id === id ? { ...n, completed: !n.completed } : n))
  ...
  const moveNote = async (id, direction, completed) => {
    ...
    await Promise.all(
      newSection.map((note, i) => api.patch(`notes/${note.id}/`, { position: i }))
    );

    const updatedSection = newSection.map((note, i) => ({ ...note, position: i }));
    const otherSection = notes.filter((n) => n.completed !== completed);
    setNotes([...otherSection, ...updatedSection].sort((a, b) => a.position - b.position));
  };

  const activeNotes = notes.filter((n) => !n.completed).sort((a, b) => a.position - b.position);
  const finishedNotes = notes.filter((n) => n.completed).sort((a, b) => a.position - b.position);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-white to-purple-100 flex flex-col items-center py-10 px-6">
      ...
      <div className="text-center mb-10">
        <h1 className="text-5xl font-extrabold text-indigo-600 drop-shadow-sm mb-2">Notely</h1>
      ...
      <div className="w-full max-w-5xl mb-10">
        ...
        {activeNotes.length === 0 ? (
          <p className="text-gray-400 italic text-center">No tasks yet — add one above!</p>
        ) : (
      ...
      <div className="w-full max-w-5xl mb-10">
        ...
        {finishedNotes.length === 0 ? (
          <p className="text-gray-400 italic text-center">No finished tasks yet!</p>
        ) : (
